...
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Retail_Media/v2026-preview/docs/Migrated to stable (2026.01)/onsite-sponsored-products-line-items.md
description: This guide documents the new "Adaptive CPC", Conquesting and Schedule Legs capabilities for Sponsored Products Line Items endpoints (available only in Preview)
hidden: true
---
...
<Info>
  **Get Started**
  ...
  Learn more about how open auction line items work with our API in [Line Items](/retail-media/docs/line-items).
</Info>
...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>
     Line item name, must be unique within the <a href="/retail-media/docs/campaigns">Campaign</a>
    </p>
    <p>
...
Results are paginated using `offset` and `limit` query parameters; if omitted, defaults to `0` and `500`, respectively. See [API Response](/retail-media/experimental/docs/api-response#pagination).

<EndpointBadge method="get">
```http
https://api.criteo.com/experimental/retail-media/campaigns/{campaignId}/auction-line-items
```
</EndpointBadge>
...
```bash cURL
curl -L -X GET "https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items?offset=0&limit=500" \
...
conn.request("GET", "/experimental/retail-media/campaigns/544937665113018368/auction-line-items?offset=0&limit=500", headers=headers)
...
Request request = new Request.Builder()
    .url("https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items?offset=0&limit=500")
    .method("GET", null)
...
$request->setUrl('https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items?offset=0&limit=500');
...
curl -L -X POST "https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items" \
    -H "Authorization: Bearer <MY_ACCESS_TOKEN>" \
...
conn.request("POST", "/experimental/retail-media/campaigns/544937665113018368/auction-line-items", payload, headers)
...
Request request = new Request.Builder()
    .url("https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items")
    .method("POST", body)
    .addHeader("Authorization", "Bearer <MY_ACCESS_TOKEN>")
...
require_once 'HTTP/Request2.php';
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/experimental/retail-media/campaigns/544937665113018368/auction-line-items');
...
<EndpointBadge method="get">
```http
https://api.criteo.com/experimental/retail-media/auction-line-items/{lineItemId}
```
</EndpointBadge>
...
```bash cURL
curl -L -X GET "https://api.criteo.com/experimental/retail-media/auction-line-items/6854840188706902009" \
    -H "Authorization: Bearer <MY_ACCESS_TOKEN>" \
    -H "Accept: application/json"
```
...
conn.request("GET", "/experimental/retail-media/auction-line-items/6854840188706902009", headers=headers)
...
Request request = new Request.Builder()
    .url("https://api.criteo.com/experimental/retail-media/auction-line-items/6854840188706902009")
    .method("GET", null)
    .addHeader("Authorization", "Bearer <MY_ACCESS_TOKEN>")
    .addHeader("Accept", "application/json")
    .build();
...
require_once 'HTTP/Request2.php';
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/experimental/retail-media/auction-line-items/6854840188706902009');
$request->setMethod(HTTP_Request2::METHOD_GET);
$request->setConfig([
    'follow_redirects' => TRUE
]);
...
```bash cURL expandable
curl -L -X PUT "https://api.criteo.com/experimental/retail-media/auction-line-items/6854840188706902009" \
    -H "Authorization: Bearer <MY_ACCESS_TOKEN>" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{
...
conn.request("PUT", "/experimental/retail-media/auction-line-items/6854840188706902009", payload, headers)
