...
Balances come in two funding models: **self-funded**, where the advertiser deposits funds directly, and **retailer-billed**, where the retailer funds the balance instead. A retailer-billed balance is scoped to a single retailer via `retailerId`, and that scoping propagates to every campaign and line item drawing on it — a retailer-billed balance can only fund campaigns for its own retailer, not an advertiser's full campaign roster. See the [Campaign Management Overview](/retail-media/docs/campaign-management-overview) for how this constraint fits into the wider entity model.
