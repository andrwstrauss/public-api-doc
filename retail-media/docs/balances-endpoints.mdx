...
<Warning>
  ...
  * All other new fields (`retailerId`, `privateMarketBillingType`) are additive. Retailer-billed balances are hidden by default on prior API versions.
  ...
  You can learn more about retailer-billed balances [on this page](/retail-media/docs/balances#retailer-billed-balances).
</Warning>
...
Results are paginated using `offset` and `limit` query parameters; if omitted, defaults to `0` and `25`, respectively. See [API Response](/criteo-apis/docs/api-response#pagination).
...
<Warning>
  On API version `2026-07` and later, retailer-billed balances are included and `retailerId`, `retailerPoNumber`, and `criteoPoNumber` are returned. The old `poNumber` field is removed.  
  Learn more about retailer-billed balances [here](/retail-media/docs/balances#retailer-billed-balances).
...
```bash cURL
curl -X GET "https://api.criteo.com/{version}/retail-media/accounts/18446744073709551616/balances?offset=0&limit=25" \
...
url = "https://api.criteo.com/{version}/retail-media/accounts/18446744073709551616/balances?offset=0&limit=25"
...
$request->setUrl('https://api.criteo.com/{version}/retail-media/accounts/18446744073709551616/balances?offset=0&limit=25');
...
<Warning>
  This endpoint only accepts `deltaAmount`, `poNumber`, and `memo`. Any other parameters (e.g. `endDate`) are silently ignored — no error is returned, but the value is not applied.
</Warning>
...
Results are paginated using `offset` and `limit` query parameters; if omitted, defaults to `0` and `25`, respectively — see [API Response](/criteo-apis/docs/api-response)
...
<table>
 ...
 <tbody>
  <tr>
   ...
   <td>
    <p>
     <a href="/retail-media/docs/campaigns">
      Campaign
     </a>
...
```bash cURL
curl -X GET "https://api.criteo.com/{version}/retail-media/balances/14094543095747588032/campaigns?offset=0&limit=25" \
    -H "Authorization: Bearer <MY_ACCESS_TOKEN>"
```
...
Request request = new Request.Builder()
  .url("https://api.criteo.com/{version}/retail-media/balances/14094543095747588032/campaigns?offset=0&limit=25")
  .method("GET", body)
  .addHeader("Accept", "application/json")
  .addHeader("Authorization", "Bearer <MY_ACCESS_TOKEN>")
  .build();
...
require_once 'HTTP/Request2.php';
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/{version}/retail-media/balances/14094543095747588032/campaigns?offset=0&limit=25');
$request->setMethod(HTTP_Request2::METHOD_GET);
$request->setConfig(array(
  'follow_redirects' => TRUE
));
