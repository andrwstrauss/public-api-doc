---
title: Catalogs
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Retail_Media/v2026_01/docs/Guides for Retail Media API/catalogs/index.md
---

<Card title="Catalog Endpoints" icon="arrow-up-right-from-square" href="/retail-media/docs/catalog-endpoints" horizontal />

A catalog is a snapshot of all catalog products from the brands and retailers associated with an account, typically updated once a day. Each account has a single catalog listing all the products that the account is eligible to promote.

<Info>
  **Important to Know**

  * Catalogs are requested and retrieved through asynchronous endpoints.

  * Product attributes are scoped to the retailer, except for universal identifiers such as GTIN, if available.

  * Brand attributes are standardized across retailers.

  * Catalogs are provided in the currency of the retailer.

  * Catalog outputs are UTF-8 encoded.

  * Catalogs are returned as a [newline-delimited JSON byte stream](https://en.wikipedia.org/wiki/JSON_streaming)
</Info>

***

## Relationships

Each [promoted product](/retail-media/docs/promoted-products) attached to a [line item](/retail-media/docs/line-items) references an entry in the account's catalog — the catalog reference lives on the product, not the line item itself. What's synced here determines what's available to promote. See the [Campaign Management Overview](/retail-media/docs/campaign-management-overview) for how the catalog fits into the wider entity model.

***

<br />


## What's next

* [Catalog Endpoints](/retail-media/docs/catalog-endpoints)
